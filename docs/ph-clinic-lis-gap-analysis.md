...
| RBAC                                   | 🟡     | `libs/shared-types/src/lib/roles.ts`                                  | 8 roles / 21 actions; `MEDICAL_TECHNOLOGIST` + `PATHOLOGIST` landed with RA 5527 supervision, no `LAB_RECEPTION` or cashier role |
...
| Audit log                              | ✅     | `AuditLog` + `AuditInterceptor`                                       | Append-only ✅ since `20260924090100`, asserted in CI since the fix in §1; before/after and `reason` since `20260925180000`      |
...
## 6.12 QC / QA — ✅ BUILT (was 🔴)

> **Status: both halves built.** Internal QC (`20260925100000_lis_quality_control`)
> and external quality assessment (`20260925140000_lis_eqap`). They answer different
> questions and neither substitutes for the other, which is why this section needed both.
>
> **Internal QC — built.** (`20260925100000_lis_quality_control`). `QcMaterial` (keyed by name AND lot, because control material is made in batches and the target mean shifts between them), `QcTarget` (mean, SD, effective window) and `QcRun` (value, z-score, outcome, violated rules, corrective action).
>
...
>
> **External quality assessment — built.**
>
...
>
> **Still open:** the Levey-Jennings chart itself, which is a UI concern over `QcRun`
> rows rather than a data gap, and the within-run 2-2s variant noted above.
...
> **Status: built** (`20260925160000_lis_supervision`). The two roles landed earlier; what was still missing was the relationship between them, which is the part the statute is actually about. A technologist could release a result to the chart and sign the report, and nothing recorded under whose authority — an inspector asking "who supervised this result" had no answer, and neither did the technologist.
>
> Every release now carries the supervising pathologist, snapshotted onto the append-only result history and onto the report signature. The pathologist of record was already captured on the laboratory profile as free text — deliberately, because they are frequently a visiting consultant with no account here — but had never been printed on a report until now. The document named who ran the laboratory and not who was answerable for its results.
>
> Four decisions worth knowing:
>
> - **A pathologist and a physician release on their own authority.** Naming a supervisor for a pathologist's own release would misstate what happened, and RA 5527's own exception covers the physician: a doctor releasing a result in their own clinic is the authorised person, not someone needing authorisation.
> - **`OWNER` is treated as needing supervision.** The role describes who pays for the subscription, not who holds a licence. A clinic whose owner is also the physician should release as `DOCTOR` — the role is what goes on the document either way.
> - **Recorded always, refused only on request.** `labSupervision.required` defaults off, for the reason the verification policy does: a clinic that has not filled in its laboratory profile would otherwise be unable to release anything, and results that never reach the ordering doctor are the worse failure. The stamp is applied either way, so the evidence is complete whichever way the switch is set.
> - **An unsupervised release is stated on the report, not omitted** — in red, the same decision as an expired Licence to Operate. A document that quietly leaves it out reads as compliance.
>
> The supervisor is snapshotted rather than read through a relation, so replacing the pathologist next year cannot rewrite who supervised last year's results; both tables hold no UPDATE or DELETE for the application role, so neither can anything else.
>
> **One consequence to be aware of.** A signature written before this migration has no supervisor recorded, and nothing distinguishes "released with no supervising pathologist" from "released before the column existed". Re-rendering such a report therefore prints the unsupervised notice on a document that did not carry it when it was issued. The notice is worded as a statement about the record rather than about the laboratory — there is no supervising pathologist _of record_ for that release, which is true either way — and the alternative, suppressing it for older rows, would mean the one case where the evidence is genuinely missing is also the one case the report stays silent about. It is called out here because the content hash covers the results, not the signature block, so this changes what an old report says without superseding it.
>
> **Still open: the PRC licence is not verified against the PRC register, and nothing checks that the pathologist of record's own licence is current.**
...
| S6  | Audit log records no before/after values and no reason **— FIXED** (`20260925180000_audit_change_detail`)                                                                                                                      | P1  | `AuditLog` model                                        |
...
> **S6 status: built** (`20260925180000_audit_change_detail`). The trail was already append-only and complete about the act — who, when, from where, which entity id — and said nothing about the content of the change. "patient.update on p_123" is not an answer to the question an audit log exists to answer, and reconstructing one meant diffing database backups.
>
> `audit_logs` now carries `changes` — an array of `{field, before, after}` — and `reason`. The diff and redaction rules are a pure module in `apps/api/src/audit/changes.ts` with 31 unit tests. Services report what they changed through the request context that already carries the tenant, so nothing had to be threaded through service signatures; the interceptor drains it when the request succeeds and **discards it when the request throws**, because a rolled-back transaction changed nothing and attaching the diff would assert a change that never happened.
>
> Three decisions worth knowing:
>
> - **The diff is taken against the caller's intent, not the row read back.** Diffing the written row reports every column the database touched, starting with `updatedAt` on every single update. Driving it from the DTO also means a request that sends one field is not recorded as clearing all the others.
> - **Values are redacted by field name, at every depth.** The name is always kept — "the password changed" is exactly what the trail should say — and the value never is. The match is a case-insensitive substring and deliberately over-eager: a field redacted unnecessarily costs a less informative row, a field missed costs a credential in a table that holds no UPDATE and no DELETE.
> - **An empty diff is stored as SQL NULL, not `[]`**, with a CHECK constraint refusing the empty array. A diff that ran and found nothing reads the same as no diff at all, and having two representations means every query for "rows that changed something" has to know about both.
>
> **Two bugs this found, both caught before merge.**
>
> The CHECK constraint immediately broke every audit write that had no changes — every login, register and create. Handing JavaScript `null` to a Prisma `Json?` field writes JSON `null` (`'null'::jsonb`), not SQL NULL, so `changes IS NULL` was false and `jsonb_typeof()` returned `'null'`. `AuditService` swallows write failures by design, so the only symptom was rows quietly not being there. The column is now omitted rather than nulled, and a regression test asserts the SQL NULL **in SQL** — over the wire both nulls arrive as `null`, so a JavaScript assertion cannot tell them apart and would have passed while the bug was live.
>
> The second was a leak. Redaction originally matched only the top-level field name, and tenant settings carry a free-form `extras` blob that is diffed one level down — so the field name was `extras` and the value was the whole object, and anything a tenant kept in there would have been written verbatim into a table that cannot be edited. Redaction now walks the whole structure, and an e2e test puts an API key in `extras` and reads the row back to prove it.
>
> **Still open: this is wired on four routes** — patient update, member role change, tenant settings and result amendment. The other audited routes record the act without the detail, which is the same state as before for them, and S7's missing actions are also still missing.
...
| Laboratory headed/managed by a certified pathologist (limited exception for a trained physician in primary/secondary labs where none available) | RA 4688; DOH AO 2021-0037                                                                                                                                                                                                                                                     | **LEGAL**                    | `Laboratory.headName` / `pathologistName` with PRC numbers, printed on the report (`20260924300000`, `20260925160000`)                                              | ✅  | Check the named pathologist's licence is current                   | P2                          |
| Medical technologists practise under pathologist supervision                                                                                    | [RA 5527](https://elibrary.judiciary.gov.ph/thebookshelf/showdocs/2/7439) ([PRC text](https://www.prc.gov.ph/sites/default/files/Medical%20Technology%20-%20Board%20Law_0.PDF))                                                                                               | **LEGAL**                    | Supervising pathologist snapshotted on every release and report signature (`20260925160000_lis_supervision`); `labSupervision.required` refuses an unsupervised one | ✅  | Verify the PRC number against the PRC register                     | done                        |
...
| Documented internal QC, QC reports per test on file                                                                                             | DOH AO 2021-0037                                                                                                                                                                                                                                                              | **REGULATORY GUIDANCE**      | `QcMaterial` / `QcTarget` / `QcRun` + Westgard evaluator (`20260925100000_lis_quality_control`)                                                                     | ✅  | Levey-Jennings chart over `QcRun` rows (UI)                        | done                        |
...
| 6   | Add `MEDICAL_TECHNOLOGIST`, `PATHOLOGIST`, `LAB_RECEPTION`, `CASHIER` roles and the `LAB_*` action vocabulary **— PARTIALLY DONE.** `MEDICAL_TECHNOLOGIST` + `PATHOLOGIST` + `LAB_RESULT_ENTER`/`LAB_RESULT_VERIFY` landed, and `20260925160000_lis_supervision` added the RA 5527 supervision relationship the roles alone could not express; **`LAB_RECEPTION` and `CASHIER` still open**, and the specimen routes still gate on `CONSULT_WRITE` | `libs/shared-types/src/lib/roles.ts`                                                                                                    | RA 5527 supervision and separation of duties cannot be expressed with 6 roles                                                                                                                                                                 |
...
| 15  | ~~Build `PatientEntitlement` + `DiscountRule` + VAT-first statutory arithmetic~~ **DONE** — `20260924280000_ph_statutory_discounts`. Rates are per-tenant `DiscountRule` rows defaulting to the statute; the VAT-before-percentage ordering is fixed in code, because it determines the deductible amount rather than being a policy choice. Senior + PWD grant one discount, not two                                                              | `apps/api/src/billing/ph-statutory.ts`, `billing.service.ts`                                                                            | RA 9994 / RA 10754 are legal requirements a billing clinic cannot skip                                                                                                                                                                        |
...
| 18  | Capture before/after values and a `reason` in the audit log **— DONE.** `20260925180000_audit_change_detail` adds `changes` + `reason`; the diff and redaction rules are a pure module in `apps/api/src/audit/changes.ts` with 31 unit tests. Wired on patient update, member role change, tenant settings and result amendment; **the other audited routes record the act without the detail**                                                    | `libs/db/prisma/schema.prisma:1359` (`AuditLog`), `apps/api/src/audit/audit.interceptor.ts`                                             | Append-only was already right; it recorded _that_ something changed, not _what_                                                                                                                                                               |
...
| 20  | ~~Add the RLS coverage assertion + the clinical boundary test matrix to CI~~ **DONE** — `privilege-coverage.spec.ts`: RLS enabled _and forced_, a policy that actually references `current_tenant_id()`, an explicit accepted list for unprotected tables, the app role not being `BYPASSRLS`, and every migration `REVOKE` still holding. It found a third instance of the default-privileges trap on its first run (§4 P0-6)                     | `apps/api-e2e/src/modules/privilege-coverage.spec.ts`, `portal-boundary.spec.ts`                                                        | A SQL assertion that every `tenantId` table has `relrowsecurity = true` would have caught #12                                                                                                                                                 |
...
| P0-5 DOH-licensable laboratory                      | closed      | catalogue, specimens, verification chain, signed reports, licence profile, referrals, equipment, EQAP, internal QC, pathologist supervision |
...
| EQAP enrolment, rounds and participation            | built       | `20260925140000_lis_eqap`                                                                                                                   |
