...
- **The clinical laboratory was two tables** at the time of the audit: `LabOrder` + `LabOrderItem`, with the result a free-text string inline on the order item. No test catalog, no specimen, no accession number, no reference-range entity, no verification or validation step, no QC, no equipment, no reagent lot, no referral laboratory, no sections, no TAT targets, no critical-value rule or acknowledgement.
  ...
  _Since then_ the catalogue (`LaboratoryTest`, `TestComponent`, `LabSection`), specimens with atomic accession numbering and append-only rejections, configured critical-value rules, and acknowledged/escalated critical-result notifications have shipped. The result verification chain is in review. **What remains unbuilt is still enough that CLINIQ cannot yet legally operate a DOH-licensed clinical laboratory**: QC, EQAP evidence, equipment and reagent lots, the laboratory licence profile and service capability, referral laboratories, and signed reports. See §6 and §30.
...
| RBAC                                   | 🟡     | `libs/shared-types/src/lib/roles.ts`                                | 6 roles / 19 actions; lab roles + `LAB_RESULT_*` in review, no cashier role                                     |
...
| **Clinical laboratory**                | 🟡     | + catalogue, specimens, critical-value rules (§6)                   | Was `LabOrder`/`LabOrderItem` only. QC, equipment, licence profile and reports still missing                    |
...
| Unit tests (API)                       | 🔴     | 9 spec files / 250+ source files                                    | Still ~4%. The new ones are the pure-logic modules (`flagging`, `accession`) — the pattern to follow            |
...
>
> **The rest of P0-2 is in review, not merged.** A result verification chain — `PENDING → PRELIMINARY → FINAL` with `CORRECTED`, an append-only `lab_result_versions` history, `enteredById`/`verifiedById` on every result, and `MEDICAL_TECHNOLOGIST` / `PATHOLOGIST` roles — is open as a pull request against `main` but is **not** part of the branch this section describes. Until it merges, a released result can still be silently rewritten and the audit log records only `lab.resultRecord` with the item id, not the old and new values.
>
...
> | Result verification chain — **in review, not merged**                                                  | `LabReport` + signatures; a standalone `ReferenceRange` entity |
>
...
## 6.8 Verification / validation workflow — 🟡 IN REVIEW (was 🔴)

> **Status: open as a pull request against `main`, not merged.** Treat this section as describing the current state of `main` until it lands.
>
> What it introduces: `PENDING → PRELIMINARY → FINAL` with `CORRECTED` (HL7's P/F/C vocabulary), an append-only `lab_result_versions` table so a corrected value no longer destroys the one a clinician may have acted on, `enteredById`/`verifiedById` on every result, and the `MEDICAL_TECHNOLOGIST` / `PATHOLOGIST` roles with `LAB_RESULT_ENTER` / `LAB_RESULT_VERIFY`.
>
> It deliberately does **not** implement the four-stage `ENTERED → VERIFIED → VALIDATED → RELEASED` chain sketched below. Technical verification and pathologist validation collapse into one release step, and whether that step requires a second person is a per-tenant setting defaulting off. The reasoning is in §4 P0-2.
...
## 6.10 Laboratory sections & service capability — 🔴
...
| 5   | Build the LIS result chain **— IN REVIEW.** Landed as `PENDING→PRELIMINARY→FINAL`/`CORRECTED` on `LabOrderItem` + append-only `lab_result_versions`, not a separate `LabResult`/`LabResultComponent` pair; verification and four-eyes are per-tenant settings defaulting off (§4 P0-2)                                          | `libs/db/prisma/schema.prisma`, `apps/api/src/lis/results/` (new)                                                                       | Today one `CONSULT_WRITE` call enters and reports a result, rewritable with no history                                                                                                                                                        |
| 6   | Add `MEDICAL_TECHNOLOGIST`, `PATHOLOGIST`, `LAB_RECEPTION`, `CASHIER` roles and the `LAB_*` action vocabulary **— PARTIALLY, IN REVIEW.** `MEDICAL_TECHNOLOGIST` + `PATHOLOGIST` + `LAB_RESULT_ENTER`/`LAB_RESULT_VERIFY` only; `LAB_RECEPTION` and `CASHIER` still open, and the specimen routes still gate on `CONSULT_WRITE` | `libs/shared-types/src/lib/roles.ts`                                                                                                    | RA 5527 supervision and separation of duties cannot be expressed with 6 roles                                                                                                                                                                 |
...
Every row below was verified against the source on the branch named, not inferred from a commit message. `main` here means merged; "in review" means an open pull request whose contents are **not** in `main`.
...
| P0-2 result verification chain  | **in review**    | not in `main`                                                                                   |
...
| P0-5 DOH-licensable laboratory  | partially closed | catalogue + specimens; QC, equipment, licence profile, reports still open                       |
