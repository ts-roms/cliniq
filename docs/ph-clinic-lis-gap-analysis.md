...
- An **append-only audit log** — _as of 2026-09-24_. The creating migration granted `SELECT, INSERT` only and said so, but `20260501000001_rls_policies` had already run `ALTER DEFAULT PRIVILEGES ... GRANT SELECT, INSERT, UPDATE, DELETE`, which applies to every table created afterwards. A narrower GRANT does not subtract; only REVOKE does, so the trail was modifiable and deletable by the app role until `20260924090100_append_only_grants` revoked it. **This entry originally reported the control as already in force — that was wrong.**
...
- **The clinical laboratory is two tables.** `LabOrder` + `LabOrderItem`, with the result stored as a free-text string inline on the order item. There is no test catalog, no specimen, no accession number, no reference-range entity, no verification or validation step, no QC, no equipment, no reagent lot, no referral laboratory, no sections, no TAT targets, and no critical-value rule or acknowledgement. **CLINIQ today cannot legally operate a DOH-licensed clinical laboratory.**
- **The ~35 `Lab*` models are a dental laboratory marketplace**, not a clinical lab. `LabCase`, `LabProduct`, `LabMaterialLot`, `LabShipment`, `LabTreatmentPlan`, `LabInvoice`, `LabCaseDispute` model crown-and-bridge manufacturing workflow between a clinic and a dental lab. This is a substantial, well-built module — and it is a naming collision that will confuse every engineer who joins after this. `apps/api/src/lab/` (dental) and `apps/api/src/labs/` (clinical) differ by one character.
...
Not a bug — a scope gap, listed as P0 because it is the stated product goal. Absent: test catalog, specimen/accession, reference-range entity, QC, EQAP evidence, equipment, reagent lots, laboratory licence profile, service capability, referral laboratory, report signatures. See §6 and §21.
...
## 6.4 Test catalog — 🔴 MISSING ENTIRELY
...
## 6.6 Specimen management — 🔴 MISSING
...
## 6.8 Verification / validation workflow — 🔴
...
```
apps/api/src/lab/         → apps/api/src/dental-lab/
apps/web/app/lab/         → apps/web/app/dental-lab/
apps/web/features/lab/    → apps/web/features/dental-lab/
apps/mobile/src/features/lab/ → .../dental-lab/
