...
@Injectable()
export class DentalLabDisputesService {
  private readonly logger = new Logger(DentalLabDisputesService.name);
  ...
  async open(
    ...
    const dispute = await this.prisma.withTenant(
      ...
      async (tx) => {
        ...
        // the existing thread.
        const open = await tx.dentalLabCaseDispute.findFirst({
          where: {
            caseId,
            status: DentalLabCaseDisputeStatus.OPEN,
            deletedAt: null,
          },
          select: { id: true },
        });
        ...
        return tx.dentalLabCaseDispute.create({
          data: {
            caseId,
  ...
  async listForCase(caseId: string, user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      if (!dentalLabCase) throw new NotFoundException('case not found');
      return tx.dentalLabCaseDispute.findMany({
  ...
  async findById(id: string, user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      const dispute = await tx.dentalLabCaseDispute.findFirst({
  ...
  async addMessage(id: string, body: string, user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      if (!dispute) throw new NotFoundException('dispute not found');
      if (dispute.status !== DentalLabCaseDisputeStatus.OPEN) {
        throw new BadRequestException(
      ...
      return tx.dentalLabCaseDisputeMessage.create({
        data: {
          disputeId: id,
  ...
  private async notifyDisputeOpened(
    ...
    const d = await this.prisma.withPlatformContext((tx) =>
      tx.dentalLabCaseDispute.findFirst({
        where: { id: disputeId },
        include: {
          case: {
            select: {
