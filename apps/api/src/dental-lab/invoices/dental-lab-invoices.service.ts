...
const TERMINAL_STATUSES: ReadonlySet<DentalLabInvoiceStatus> = new Set([
  DentalLabInvoiceStatus.PAID,
  DentalLabInvoiceStatus.VOID,
]);

@Injectable()
export class DentalLabInvoicesService {
  private readonly logger = new Logger(DentalLabInvoicesService.name);

  constructor(
    private readonly prisma: PrismaService,
    private readonly links: DentalLabClinicLinksService,
    private readonly pdf: DentalLabPdfRenderingService,
        ...
        tx.dentalLabInvoice.findMany({
          where: {
            ...
            status: filter.status ?? {
              in: [
                DentalLabInvoiceStatus.ISSUED,
                DentalLabInvoiceStatus.PAID,
                DentalLabInvoiceStatus.OVERDUE,
                DentalLabInvoiceStatus.VOID,
              ],
            },
          },
  ...
  async findById(id: string, user: AuthenticatedUser) {
    const found = await this.prisma.withTenant(
      ...
      user.userId,
      async (tx) => {
        const invoice = await tx.dentalLabInvoice.findFirst({
        ...
        if (
          invoice.status === DentalLabInvoiceStatus.DRAFT &&
  ...
  async createDraft(dto: CreateLabInvoiceDto, user: AuthenticatedUser) {
    ...
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      return tx.dentalLabInvoice.create({
        data: {
          labTenantId: user.tenantId,
   ...
   * case becomes one line item priced from `DentalLabCase.unitPrice`. Cases must
  ...
  async generateFromCases(dto: GenerateFromCasesDto, user: AuthenticatedUser) {
    ...
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      const cases = await tx.dentalLabCase.findMany({
        where: {
          id: { in: dto.caseIds },
          labTenantId: user.tenantId,
          clinicTenantId: dto.clinicTenantId,
      ...
      const wrongStatus = cases.find(
        (c) => c.status !== DentalLabCaseStatus.DELIVERED,
      );
      if (wrongStatus) {
        throw new BadRequestException(
      ...
      const already = await tx.dentalLabInvoiceItem.findMany({
        where: {
          caseId: { in: dto.caseIds },
          invoice: {
            status: { not: DentalLabInvoiceStatus.VOID },
            deletedAt: null,
          },
        },
  ...
  async update(id: string, dto: UpdateInvoiceDto, user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      return tx.dentalLabInvoice.update({
        where: { id },
        data: updates,
        include: { items: true, paymentLinks: true },
      });
  ...
  async addItem(
    ...
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      const item = await tx.dentalLabInvoiceItem.create({
        data: {
          invoiceId,
          caseId: it.caseId ?? null,
          description: it.description,
  ...
  async updateItem(
    ...
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      const existing = await tx.dentalLabInvoiceItem.findFirst({
        where: { id: itemId, invoiceId },
      });
      if (!existing) throw new NotFoundException('item not found');
      const qty = dto.qty ?? existing.qty;
      const unitPrice = dto.unitPriceCents ?? existing.unitPriceCents;
      const updated = await tx.dentalLabInvoiceItem.update({
        where: { id: itemId },
        data: {
  ...
  async deleteItem(invoiceId: string, itemId: string, user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      if (!existing) throw new NotFoundException('item not found');
      await tx.dentalLabInvoiceItem.delete({ where: { id: itemId } });
      await this.recomputeTotals(tx, invoiceId);
    });
  }
  ...
  async issue(id: string, user: AuthenticatedUser) {
    const updated = await this.prisma.withTenant(
      ...
      async (tx) => {
        ...
        const max = await tx.dentalLabInvoice.aggregate({
          where: { labTenantId: user.tenantId },
          _max: { refNumber: true },
        });
        const refNumber = (max._max.refNumber ?? 0) + 1;
        const result = await tx.dentalLabInvoice.update({
          where: { id },
          data: {
  ...
  async recordPayment(
    ...
    const { invoice, fullyPaid } = await this.prisma.withTenant(
      ...
      user.userId,
      async (tx) => {
        const existing = await tx.dentalLabInvoice.findFirst({
        ...
        if (existing.status === DentalLabInvoiceStatus.DRAFT) {
          throw new BadRequestException(
        ...
        const result = await tx.dentalLabInvoice.update({
          ...
          data: {
            paidCents: newPaid,
            status: paidNow ? DentalLabInvoiceStatus.PAID : existing.status,
  ...
  async voidInvoice(id: string, user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      if (!invoice) throw new NotFoundException('invoice not found');
      if (invoice.status === DentalLabInvoiceStatus.PAID) {
        throw new BadRequestException('paid invoices cannot be voided');
      }
      if (invoice.status === DentalLabInvoiceStatus.VOID) return invoice;
      return tx.dentalLabInvoice.update({
        where: { id },
        data: { status: DentalLabInvoiceStatus.VOID, voidedAt: new Date() },
        include: { items: true, paymentLinks: true },
      });
    });
  }
  ...
  async createPaymentLink(
    ...
    const reservation = await this.prisma.withTenant(
      ...
      async (tx) => {
        ...
        if (!invoice) throw new NotFoundException('invoice not found');
        if (
          invoice.status !== DentalLabInvoiceStatus.ISSUED &&
          invoice.status !== DentalLabInvoiceStatus.OVERDUE
        ) {
          throw new BadRequestException(
    ...
    const provider = dto.provider ?? DentalLabPaymentLinkProvider.MANUAL;
    if (provider === DentalLabPaymentLinkProvider.PAYMONGO) {
      ...
      // Both names are null under RLS (see DentalLabCounterpartyService) and this
    ...
    return this.prisma.withTenant(user.tenantId, user.userId, (tx) =>
      tx.dentalLabPaymentLink.create({
        data: {
          invoiceId,
          provider,
          amountCents: reservation.amount,
          ...
          status: DentalLabPaymentLinkStatus.PENDING,
        },
      }),
    );
  }
  ...
  async cancelPaymentLink(
    ...
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      if (!invoice) throw new NotFoundException('invoice not found');
      const link = await tx.dentalLabPaymentLink.findFirst({
        where: { id: linkId, invoiceId },
      });
      if (!link) throw new NotFoundException('payment link not found');
      if (link.status !== DentalLabPaymentLinkStatus.PENDING) {
        throw new BadRequestException(`cannot cancel a ${link.status} link`);
      }
      return tx.dentalLabPaymentLink.update({
        where: { id: linkId },
        data: { status: DentalLabPaymentLinkStatus.CANCELLED },
      });
    });
  }
   ...
   * `DentalLabPaymentLink` by `externalId`, marks it PAID, and applies the
   ...
   * tenant from `DentalLabPaymentLink.invoice.labTenantId`.
   */
  async handlePaymongoEvent(event: {
    ...
    const link = await this.prisma.withPlatformContext((tx) =>
      tx.dentalLabPaymentLink.findFirst({
    ...
    if (link.status === DentalLabPaymentLinkStatus.PAID) {
    ...
    await this.prisma.withTenant(link.invoice.labTenantId, null, async (tx) => {
      await tx.dentalLabPaymentLink.update({
        where: { id: link.id },
        data: {
          status: DentalLabPaymentLinkStatus.PAID,
          paidAt: new Date(),
        },
      });
      const inv = await tx.dentalLabInvoice.findFirst({
      ...
      invoiceId = inv.id;
      await tx.dentalLabInvoice.update({
        where: { id: inv.id },
        data: {
          paidCents: capped,
          status: fullyPaid ? DentalLabInvoiceStatus.PAID : inv.status,
  ...
  async runMonthlySweep(period: string | null, user: AuthenticatedUser) {
    ...
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      const candidates = await tx.dentalLabCase.findMany({
        where: {
          labTenantId: user.tenantId,
          deletedAt: null,
      ...
      for (const [clinicTenantId, cases] of byClinic) {
        ...
        const subtotal = lines.reduce((sum, it) => sum + it.amountCents, 0);
        const inv = await tx.dentalLabInvoice.create({
          data: {
            labTenantId: user.tenantId,
            clinicTenantId,
            status: DentalLabInvoiceStatus.DRAFT,
  ...
  async generatePdfAsLab(id: string, user: AuthenticatedUser) {
    ...
    // arrive null under RLS (see DentalLabCounterpartyService), which made every
  ...
  async getPdfAsClinic(id: string, user: AuthenticatedUser) {
    ...
    if (!invoice) throw new NotFoundException('invoice not found');
    if (invoice.status === DentalLabInvoiceStatus.DRAFT) {
      throw new NotFoundException('invoice not found');
    }
  ...
  private normalizeItem(it: DentalLabInvoiceItemInputDto): {
    caseId: string | null;
    description: string;
    qty: number;
  ...
  private async assertCaseOwnership(
    tx: Pick<PrismaService, 'dentalLabCase'>,
    ...
    const found = await tx.dentalLabCase.findMany({
      where: {
        id: { in: ids },
        labTenantId,
        clinicTenantId,
  ...
  private async loadDraftAsLab(
    tx: Pick<PrismaService, 'dentalLabInvoice'>,
    invoiceId: string,
    labTenantId: string,
  ) {
    const invoice = await tx.dentalLabInvoice.findFirst({
  ...
  private async recomputeTotals(
    tx: Pick<PrismaService, 'dentalLabInvoiceItem' | 'dentalLabInvoice'>,
    invoiceId: string,
  ): Promise<void> {
    const agg = await tx.dentalLabInvoiceItem.aggregate({
      where: { invoiceId },
      _sum: { amountCents: true },
    });
    const subtotal = agg._sum.amountCents ?? 0;
    const current = await tx.dentalLabInvoice.findFirst({
      where: { id: invoiceId },
