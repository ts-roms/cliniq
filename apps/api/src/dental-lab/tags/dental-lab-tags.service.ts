...
export class DentalLabTagsService {
  ...
  async create(dto: CreateTagDto, user: AuthenticatedUser) {
    ...
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      try {
        return await tx.dentalLabCaseTag.create({
          data: {
  ...
  async update(id: string, dto: UpdateTagDto, user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      const tag = await tx.dentalLabCaseTag.findFirst({
        where: { id, deletedAt: null },
      });
      if (!tag) throw new NotFoundException('tag not found');
      return tx.dentalLabCaseTag.update({
        where: { id },
        data: {
  ...
  async remove(id: string, user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      if (!tag) throw new NotFoundException('tag not found');
      await tx.dentalLabCaseTag.update({
        where: { id },
  ...
  async assign(caseId: string, tagId: string, user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      return tx.dentalLabCaseTagAssignment.upsert({
  ...
  async unassign(caseId: string, tagId: string, user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      if (!dentalLabCase) throw new NotFoundException('case not found');
      await tx.dentalLabCaseTagAssignment
        .delete({
          where: { caseId_tagId: { caseId, tagId } },
        })
        .catch(() => {
  ...
  async listForCase(caseId: string, user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      if (!dentalLabCase) throw new NotFoundException('case not found');
      const rows = await tx.dentalLabCaseTagAssignment.findMany({
        where: { caseId },
        include: { tag: true },
      });
