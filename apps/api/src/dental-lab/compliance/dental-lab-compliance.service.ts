 ...
 */
@Injectable()
export class DentalLabComplianceService {
  constructor(
  ...
  async listConformity(user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, (tx) =>
      tx.dentalLabConformityDocTemplate.findMany({
  ...
  async createConformity(dto: CreateTemplateDto, user: AuthenticatedUser) {
    ...
    return this.prisma.withTenant(user.tenantId, user.userId, (tx) =>
      tx.dentalLabConformityDocTemplate.create({
        data: {
  ...
  async updateConformity(
    ...
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      const t = await tx.dentalLabConformityDocTemplate.findFirst({
        where: { id, deletedAt: null },
      });
      if (!t) throw new NotFoundException('template not found');
      return tx.dentalLabConformityDocTemplate.update({
        where: { id },
        data: {
  ...
  async removeConformity(id: string, user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      if (!t) throw new NotFoundException('template not found');
      await tx.dentalLabConformityDocTemplate.update({
        where: { id },
  ...
  async listConsent(user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, (tx) =>
      tx.dentalLabConsentTemplate.findMany({
  ...
  async createConsent(dto: CreateTemplateDto, user: AuthenticatedUser) {
    await this.requireLab(user);
    return this.prisma.withTenant(user.tenantId, user.userId, (tx) =>
      tx.dentalLabConsentTemplate.create({
        data: {
          tenantId: user.tenantId,
          name: dto.name,
          body: dto.body,
  ...
  async updateConsent(
    ...
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      const t = await tx.dentalLabConsentTemplate.findFirst({
        where: { id, deletedAt: null },
      });
      if (!t) throw new NotFoundException('template not found');
      return tx.dentalLabConsentTemplate.update({
        where: { id },
        data: {
          name: dto.name ?? t.name,
          body: dto.body ?? t.body,
  ...
  async removeConsent(id: string, user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      if (!t) throw new NotFoundException('template not found');
      await tx.dentalLabConsentTemplate.update({
        where: { id },
        data: { deletedAt: new Date() },
      });
    });
  ...
  async listSignaturesForCase(caseId: string, user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      return tx.dentalLabConsentSignature.findMany({
        where: { caseId },
  ...
  async captureSignature(
    ...
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      const template = await tx.dentalLabConsentTemplate.findFirst({
        where: { id: dto.templateId, deletedAt: null },
      });
      if (!template) throw new NotFoundException('template not found');

      return tx.dentalLabConsentSignature.create({
        data: {
          caseId,
  ...
  async renderConformityPdf(
    ...
    // below is null under RLS (see DentalLabCounterpartyService) — dereferencing it
    ...
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      const template = templateId
        ? await tx.dentalLabConformityDocTemplate.findFirst({
        ...
        : await tx.dentalLabConformityDocTemplate.findFirst({
            where: {
              ...
              deletedAt: null,
              OR: [
                { productId: dentalLabCase.product.id },
                { productId: null },
              ],
            },
      ...
      const lots =
        dentalLabCase.materialUsages
          .map((u) => `${u.lot.material.name} · lot ${u.lot.lotNumber}`)
          .join('\n') || '—';
      const ref =
        dentalLabCase.refNumber !== null
          ? `#${dentalLabCase.refNumber}`
          : dentalLabCase.id.slice(-6);
      const vars: Record<string, string> = {
        caseRef: ref,
        patient: dentalLabCase.patientLabel ?? '—',
        doctor: dentalLabCase.doctorLabel ?? '—',
        labName: dentalLabCase.lab.name,
        clinicName,
        product: dentalLabCase.product.name,
      ...
      const stored = await this.pdf.renderToS3(keyPrefix, filename, (doc) => {
        this.pdf.drawLetterhead(doc, {
          labName: dentalLabCase.lab.name,
          labKindLabel:
            dentalLabCase.lab.kind === 'LAB' ? 'Dental laboratory' : undefined,
          docTitle: 'Declaration of Conformity',
          docRef: `Case ${ref} · ${dentalLabCase.product.name}`,
        });
        ...
        this.pdf.drawFooter(
          doc,
          `${dentalLabCase.lab.name} · Conformity ${ref}`,
        );
      });
