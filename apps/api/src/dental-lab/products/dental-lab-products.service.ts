...
@Injectable()
export class DentalLabProductsService {
  ...
  listCategories(user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, (tx) =>
      tx.dentalLabProductCategory.findMany({
  ...
  async createCategory(dto: CreateLabCategoryDto, user: AuthenticatedUser) {
    ...
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      if (dto.parentId) {
        const parent = await tx.dentalLabProductCategory.findFirst({
          where: { id: dto.parentId, deletedAt: null },
        });
        if (!parent) throw new BadRequestException('parent category not found');
      }
      return tx.dentalLabProductCategory.create({
        data: {
  ...
  async updateCategory(
    ...
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      const cat = await tx.dentalLabProductCategory.findFirst({
      ...
      return tx.dentalLabProductCategory.update({
        where: { id },
        data: {
  ...
  async removeCategory(id: string, user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      await tx.dentalLabProductCategory.update({
        where: { id },
  ...
  async createProduct(dto: CreateLabProductDto, user: AuthenticatedUser) {
    ...
    this.validatePricing(
      dto.pricingMode ?? DentalLabProductPricingMode.FIXED,
      dto.defaultPrice,
    );
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      if (dto.categoryId) {
      ...
      try {
        return await tx.dentalLabProduct.create({
          data: {
            tenantId: user.tenantId,
            ...
            pricingMode: dto.pricingMode ?? DentalLabProductPricingMode.FIXED,
  ...
  async updateProduct(
    ...
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      const existing = await tx.dentalLabProduct.findFirst({
        where: { id, deletedAt: null },
      });
      ...
      return tx.dentalLabProduct.update({
        where: { id },
        data: {
  ...
  async removeProduct(id: string, user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      ...
      if (!existing) throw new NotFoundException('product not found');
      await tx.dentalLabProduct.update({
        where: { id },
  ...
  async cloneProduct(id: string, user: AuthenticatedUser) {
    return this.prisma.withTenant(user.tenantId, user.userId, async (tx) => {
      const src = await tx.dentalLabProduct.findFirst({
        where: { id, deletedAt: null },
      });
      if (!src) throw new NotFoundException('product not found');
      return tx.dentalLabProduct.create({
        data: {
          tenantId: user.tenantId,
  ...
  private validatePricing(
    mode: DentalLabProductPricingMode,
    price: number | null | undefined,
  ) {
    if (
      mode === DentalLabProductPricingMode.FIXED &&
    ...
    if (
      mode === DentalLabProductPricingMode.ADJUST_ON_ORDER &&
      price !== null &&
      price !== undefined
    ) {
      throw new BadRequestException(
