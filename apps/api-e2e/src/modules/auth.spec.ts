...
describe('@org/api-e2e auth module', () => {
  ...
  describe('happy path', () => {
    it('POST /api/auth/register issues access + refresh tokens for an invited user', async () => {
      ...
      // Registration is invite-only: a slug alone is refused.
      const bare = await axiosBare.post('/api/auth/register', {
        email,
      ...
      const invite = await client.axios.post('/api/members/invites', {
        email,
        role: 'RECEPTIONIST',
      });
      expect(invite.status).toBe(201);
      const inviteToken = new URL(invite.data.inviteUrl).searchParams.get(
        'invite',
      );
      ...
      // The role comes from the invite, not from "first user wins".
    ...
    it('POST /api/auth/logout revokes the session and returns 200', async () => {
      const axiosBare = (await import('axios')).default.create({
        baseURL: env.baseUrl,
        validateStatus: () => true,
      });
  ...
  describe('failure modes', () => {
    it('register with unknown tenantSlug → 403 (invite required is checked first)', async () => {
      const axiosBare = (await import('axios')).default.create({
        baseURL: env.baseUrl,
        validateStatus: () => true,
      });
    ...
    it('register with an already-registered email → 409 (even with a fresh invite)', async () => {
      const { tenant, client } = await env.makeTenant();
      const axiosBare = (await import('axios')).default.create({
        baseURL: env.baseUrl,
        validateStatus: () => true,
      });
      // The owner's email already has an account; inviting it again is a
      // 409 from /members/invites (already a member) …
      const again = await client.axios.post('/api/members/invites', {
        email: tenant.ownerEmail,
        role: 'DOCTOR',
      });
      expect(again.status).toBe(409);
      // … and registering it via any token path is a 409 from /auth/register.
      const dup = await axiosBare.post('/api/auth/register', {
        email: tenant.ownerEmail,
        name: 'Second Time',
        password: 'TestPassword123!',
        tenantSlug: tenant.slug,
        inviteToken: 'irrelevant-the-email-check-comes-first-00000',
      });
