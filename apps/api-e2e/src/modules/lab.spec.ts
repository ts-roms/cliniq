/**
 * /api/dental-lab/* — the lab marketplace mega-module (8 sub-controllers).
 *
...
describe('@org/api-e2e lab module', () => {
  ...
  describe('happy path — clinic-links', () => {
    it('LAB OWNER can invite a clinic and list links', async () => {
      ...
      const list = await lab.client.axios.get('/api/dental-lab/clinic-links');
  ...
  describe('happy path — products', () => {
    it('LAB OWNER can create a product and list catalog', async () => {
      const create = await lab.client.axios.post('/api/dental-lab/products', {
      ...
      const list = await lab.client.axios.get('/api/dental-lab/products');
      expect(list.status).toBe(200);
      expect(Array.isArray(list.data)).toBe(true);
    });
  });
  ...
  describe('happy path — tags', () => {
    it('LAB OWNER can create + list tags', async () => {
      const create = await lab.client.axios.post('/api/dental-lab/tags', {
        name: `tag-${Date.now()}`,
        color: 'FF0000',
      });
      expect([200, 201]).toContain(create.status);

      const list = await lab.client.axios.get('/api/dental-lab/tags');
      expect(list.status).toBe(200);
    });
  });
  ...
  describe('happy path — materials', () => {
    it('LAB OWNER can create + list materials', async () => {
      const create = await lab.client.axios.post('/api/dental-lab/materials', {
        name: `Material ${Date.now()}`,
        unitOfMeasure: 'gram',
      });
      expect([200, 201]).toContain(create.status);

      const list = await lab.client.axios.get('/api/dental-lab/materials');
      expect(list.status).toBe(200);
      expect(Array.isArray(list.data)).toBe(true);
    });
  });
  ...
  describe('happy path — compliance (conformity templates)', () => {
    it('LAB OWNER can list templates (empty on a fresh tenant)', async () => {
      const list = await lab.client.axios.get(
        '/api/dental-lab/conformity-templates',
      );
      expect(list.status).toBe(200);
      expect(Array.isArray(list.data)).toBe(true);
    });
  ...
  describe('happy path — invoices', () => {
    it('LAB OWNER can list invoices (empty on a fresh tenant)', async () => {
      const list = await lab.client.axios.get('/api/dental-lab/invoices');
      expect(list.status).toBe(200);
      expect(Array.isArray(list.data)).toBe(true);
    });
  });
  ...
  describe('happy path — stats', () => {
    it('LAB OWNER can fetch stats overview', async () => {
      const res = await lab.client.axios.get('/api/dental-lab/stats');
  ...
  describe('feature gating', () => {
    it('CLINIC tenant cannot hit lab-side routes (wrong tenant kind)', async () => {
      ...
      const clinic = await env.makeTenant({ kind: 'CLINIC' });
      const res = await clinic.client.axios.get('/api/dental-lab/products');
  ...
  describe('RBAC denial', () => {
    it('DOCTOR on a LAB tenant cannot create products (TENANT_MANAGE required)', async () => {
      ...
      const res = await doctor.client.axios.post('/api/dental-lab/products', {
        name: 'doctor cannot',
        defaultPrice: 1,
        currency: 'PHP',
      });
  ...
  describe('multi-tenant isolation', () => {
    it('Lab B cannot see Lab A products', async () => {
      ...
      const created = await labA.client.axios.post('/api/dental-lab/products', {
        name: `RLS-A-${Date.now()}`,
        defaultPrice: 100,
        currency: 'PHP',
      });
      ...
      const listB = await labB.client.axios.get('/api/dental-lab/products');
  ...
  describe('authentication', () => {
    it('unauthenticated returns 401', async () => {
      ...
      const res = await axiosBare.get('/api/dental-lab/products');
