...
const KIND_LABEL: Record<DentalLabCaseDisputeKind, string> = {
  QUALITY: 'Quality / fit',
  BILLING: 'Billing',
  DELIVERY: 'Delivery',
  OTHER: 'Other',
...
const STATUS_COLOR: Record<DentalLabCaseDisputeStatus, string> = {
...
function OpenForm({
  ...
  onSubmit: (input: { kind: DentalLabCaseDisputeKind; reason: string }) => void;
  onCancel: () => void;
  pending: boolean;
  error: Error | null;
}) {
  const [kind, setKind] = useState<DentalLabCaseDisputeKind>('QUALITY');
  ...
  return (
    ...
    >
      <div className="grid gap-2 md:grid-cols-[200px,1fr]">
        <Select
          value={kind}
          onChange={(e) => setKind(e.target.value as DentalLabCaseDisputeKind)}
        >
          {(Object.keys(KIND_LABEL) as DentalLabCaseDisputeKind[]).map((k) => (
